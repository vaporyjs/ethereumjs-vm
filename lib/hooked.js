  ...
  function createAccountStorageTrie (address, cb) {
    ...
    var storageTrie = new FallbackStorageTrie({
      fetchStorage: function (key, cb) {
        hooks.fetchAccountStorage(addressHex, vapUtil.addHexPrefix(key), cb)
      }
    })
    cb(null, storageTrie)
  }
  ...
  function loadAccount (address, cb) {
    ...
    async.parallel({
      ...
      var account = new Account(results)
        // not used but needs to be anything but the default (vapUtil.SHA3_NULL)
...
function fromWeb3Provider (provider, blockNumber, opts) {
  return createHookedVm(opts, {
    fetchAccountBalance: createRpcFunction(provider, 'vap_getBalance', blockNumber),
    fetchAccountNonce: createRpcFunction(provider, 'vap_getTransactionCount', blockNumber),
    fetchAccountCode: createRpcFunction(provider, 'vap_getCode', blockNumber),
    fetchAccountStorage: createRpcFunction(provider, 'vap_getStorageAt', blockNumber)
  })
